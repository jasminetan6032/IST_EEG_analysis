...
for part = 1:22
    ...
    pcorrect_change = abs(cell2mat({trialmatrix_all.PCorrectChange})');
...
for row = 1:length(trialmatrix_all)
    x = abs(trialmatrix_all(row).PCorrectChange);
    if x <= Q_change(1) 
        trialmatrix_all(row).AbsChangeBin = 1;
    elseif x > Q_change(1) && x <= Q_change(2) 
        trialmatrix_all(row).AbsChangeBin = 2;
    elseif x > Q_change(2) && x <= Q_change(3)
        trialmatrix_all(row).AbsChangeBin = 3;
    elseif x > Q_change(3) && x <= Q_change(4)
        trialmatrix_all(row).AbsChangeBin = 4;
    elseif x > Q_change(4)
        trialmatrix_all(row).AbsChangeBin = 5;
    end
end 

figure;
subplot(2,1,1);
histogram(cell2mat({trialmatrix_all.AbsChangeBin}));
change_bins = cell2mat({trialmatrix_all.AbsChangeBin});
